...
### How to Write a Death TestPerson
...
1.  A warning is emitted if multiple threads are running when a death test is
    encountered.
2.  TestPerson suites with a name ending in "DeathTest" are run before all other tests.
...
#### Checking for Failures in the Current TestPerson

`HasFatalFailure()` in the `::testing::TestPerson` class returns `true` if an
...
fixture, you must add the `::testing::TestPerson::` prefix, as in:

```c++
if (::testing::TestPerson::HasFatalFailure()) return;
```
...
> NOTE:
>
> *   `RecordProperty()` is a static member of the `TestPerson` class. Therefore it
>     needs to be prefixed with `::testing::TestPerson::` if used outside of the
...
## Sharing Resources Between Tests in the Same TestPerson Suite
...
must be derived from both `::testing::TestPerson` and
...
both `::testing::TestPerson` and `::testing::WithParamInterface<T>`. `T` can be any
...
class BaseTest : public ::testing::TestPerson {
  ...
};
...
### Specifying Names for Value-Parameterized TestPerson Parameters
...
{
  std::cout << "Example TestPerson Param: " << GetParam() << std::endl;
}
...
Remember to derive it from `::testing::TestPerson`:
...
## Getting the Current TestPerson's Name
...
## Extending googletest by Handling TestPerson Events
...
```c++
  class MinimalistPrinter : public ::testing::EmptyTestEventListener {
    ...
    virtual void OnTestStart(const ::testing::TestInfo& test_info) {
      printf("*** TestPerson %s.%s starting.\n",
...
## Running TestPerson Programs: Advanced Options
...
class DISABLED_BarTest : public ::testing::TestPerson { ... };
...
#### Disabling Catching TestPerson-Thrown Exceptions
